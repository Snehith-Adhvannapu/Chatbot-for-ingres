...
export function Sidebar({ onSuggestedQuery, language = "en" }: SidebarProps) {
  ...
  const suggestedQueries = [
    {
      icon: MapPin,
      text: t('suggestedQueries.maharashtraStatus'),
    },
    {
      icon: TrendingUp,
      text: t('suggestedQueries.gujaratRecharge'),
    },
    {
      icon: AlertTriangle,
      text: t('suggestedQueries.punjabBlocks'),
    },
    {
      icon: History,
      text: t('suggestedQueries.rajasthanData'),
    },
  ];

  const categories = [
    { name: t('categoryNames.safe'), color: "bg-green-500", bgColor: "bg-green-100", textColor: "text-green-800" },
    { name: t('categoryNames.semiCritical'), color: "bg-yellow-500", bgColor: "bg-yellow-100", textColor: "text-yellow-800" },
    { name: t('categoryNames.critical'), color: "bg-orange-500", bgColor: "bg-orange-100", textColor: "text-orange-800" },
    { name: t('categoryNames.overExploited'), color: "bg-red-500", bgColor: "bg-red-100", textColor: "text-red-800" },
  ];
    ...
    <aside className="w-80 bg-card border-r border-border flex flex-col" data-testid="sidebar">
      ...
      <div className="flex-1 p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-md font-semibold">{t('recentSearches')}</h3>
        ...
        <div className="space-y-2">
          {recentSearches.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground italic">{t('noRecentSearches')}</p>
          )}
        </div>
      </div>
    </aside>
